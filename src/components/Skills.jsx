...
function Skills() {
  const images = [
    { src: aiImg, alt: "AI Image" },
    { src: nextImg, alt: "Next.js Image" },
    { src: pyImg, alt: "Python Image" },
  ];

  return (
    <>
      <div className="main-skill flex flex-col items-center text-center w-full min-h-[80vh] mt-12 px-6">
        {/* Section Title */}
        <motion.div
        ...
        >
          <div className="w-full max-w-xs sm:max-w-sm md:max-w-lg lg:max-w-4xl">
            <CardCarousel
              images={images.map((img) => ({
                ...img,
                className:
                  "rounded-xl object-cover w-28 h-28 sm:w-32 sm:h-32 md:w-40 md:h-40 lg:w-48 lg:h-48",
              }))}
              autoplayDelay={2000}
              showPagination={true}
              showNavigation={true}
